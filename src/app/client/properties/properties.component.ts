...
export class PropertiesComponent implements OnInit {
 ...
 testProperties: Property[] = [];
  ...
  this.ps.getAllProperties().subscribe({
   next: (data)=>this.testProperties = data,
   error: (err)=>console.log(err)
  })
  console.log('displayproperty',this.displayedProperties);

    // this.allProperties = this.ps.getAllProperties();
    ...
    this.filteredProperties = this.ps.filterShortlets(this.searchData);
    this.currentPage = 1;
    this.updateDisplayedProperties();
  });
